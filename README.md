...
É a metade do problema que a maioria dos projetos pula, e é onde eu escolhi ficar.
...
| O que a área pede | Onde está — com o número |
|---|---|
| **Recuperação (RAG)** | Híbrida BM25 + E5 multilíngue, fusão RRF sobre **posição**, não sobre score: MRR **0,644** contra 0,361 do léxico puro, recall@1 de 0,133 → **0,533** |
...
| **Chunking com propósito** | Segmentação por artigo porque é o que **torna a avaliação possível** — sem a etiqueta do dispositivo não há como cruzar citação com estado de vigência |
...
| **Monitoramento e drift** | PSI e CSI com o alarme **testado contra safra deteriorada** — e o achado publicado foi que o PSI agregado **não** disparou |
...
| **Performance de dados** | Três camadas de leitura sobre Parquet: **5M × 26 colunas em 4,8 s com 261 MB** de pico — memória plana enquanto o dado cresce 50x |
...
Não é um chatbot sobre regulação; é a medição de um. Recuperação híbrida com fusão RRF, resolução de dispositivos empilhados (o BACEN publica até **sete versões** do mesmo inciso coladas em sequência), auditoria de vigência por citação. As duas camadas que sustentam o argumento **não usam LLM nenhum**.
...
25x mais rápido e 24x mais econômico em memória que o `ydata-profiling`, com precisão de 100% e **zero falsos positivos em 100 colunas limpas** — publicado junto com a margem até o limiar, que é apertada e está registrada como limite.

`DuckDB` · `PyArrow` · `CLI empacotada`

### 🤖 [Data Insight AI](https://github.com/guilhermehrsilva/data-insight-ai)

BI self-service em que o Gemini explora o dataset, recomenda KPIs, gera 30+ tipos de gráfico e responde perguntas de negócio em linguagem natural — com modo de fallback que mantém os gráficos de pé quando a API cai.

`Gemini` · `Streamlit` · `Plotly`

### 💳 [Credit Risk Decision Engine](https://github.com/guilhermehrsilva/credit-risk-decision-engine)

**Trocar o corte de 0,50 pelo break-even econômico rendeu +24,0% na carteira de teste, com o mesmo modelo e a mesma AUC.** O corte ótimo sai de duas constantes de negócio e não olha uma linha do dataset — e ganhou do ótimo procurado por varredura, que pegou ruído da amostra.
...
Audita o benchmark e reconstrói sem os atalhos — a PR-AUC honesta cai de 1,000 para 0,406 enquanto a ROC mal se move. A decisão vira triagem sob capacidade finita: ordenar por valor em risco pega **menos** fraudes e salva mais dinheiro.
